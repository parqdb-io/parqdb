...
Browser client for immutable ParqDB IVF-LVQ4/LVQ8 artifacts hosted on public
HTTPS object storage.

```ts
import { ParqDB } from '@parqdb/browser'

const index = await ParqDB.open('https://example.com/manifest.json')
const hits = await index.search(query, { nprobe: 64, k: 10 })
```

The client fetches the exact artifact manifest, ranks all leaf centroids, reads
...
ranges exactly as fetched, deduplicates covered in-flight loads, and evicts
least-recently-used ranges at the byte budget. It never expands a cold HTTP
request for cache alignment, and a cache hit does not call `fetch`. Set
`rangeCacheBytes` when opening the index to change the budget, or set it to
zero to disable this cache. The underlying partial requests continue to use
