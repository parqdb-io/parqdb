...
def test_ivf_has_one_canonical_encoding() -> None:
    assert parqdb.IVF(1).encoding == "source"
    assert parqdb.IVF(1, encoding="source").encoding == "source"
    assert parqdb.IVF(1, encoding="lvq4").encoding == "lvq4"
    assert parqdb.IVF(1, encoding="lvq8").encoding == "lvq8"
    with pytest.raises(TypeError, match="encoding must be a string"):
        parqdb.IVF(1, encoding=cast(Any, 1))
    with pytest.raises(ValueError, match="unsupported encoding"):
        parqdb.IVF(1, encoding="pq")


def test_ivf_has_one_canonical_metric() -> None:
    assert parqdb.IVF(1).metric == "l2_squared"
    assert parqdb.IVF(1, metric="cosine").metric == "cosine"
    with pytest.raises(TypeError, match="metric must be a string"):
        parqdb.IVF(1, metric=cast(Any, 1))
    with pytest.raises(ValueError, match="unsupported metric"):
        parqdb.IVF(1, metric="dot")
...
def test_write_options_are_explicit_and_validated() -> None:
    ...
    assert parqdb.WriteOptions(partitions=4).partitions == 4
    assert parqdb.WriteOptions(compression="zstd(3)").compression == "zstd(3)"

    with pytest.raises(TypeError, match="compression must be a string"):
        parqdb.WriteOptions(compression=cast(Any, 1))
    with pytest.raises(ValueError, match="unsupported Parquet compression"):
        parqdb.WriteOptions(compression=cast(Any, "invalid"))
    with pytest.raises(ValueError, match="unsupported Parquet compression"):
        parqdb.WriteOptions(compression="zstd")
    for field in ("partitions", "target_file_size"):
        with pytest.raises(ValueError, match=field):
            parqdb.WriteOptions(**{field: 0})
...
def test_write_options_accept_supported_compression(
    ...
    assert parqdb.WriteOptions(compression=compression).compression == compression
...
def test_write_options_reject_invalid_compression_levels(
    ...
    with pytest.raises(ValueError, match="unsupported Parquet compression"):
        parqdb.WriteOptions(compression=compression)
