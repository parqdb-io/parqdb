...
use relify_index::MetadataStore;

/// One unreachable object managed by a Relify warehouse.
...
pub enum MaintenanceKind {
    /// Immutable Relify metadata file.
    Metadata,
        ...
        if !candidate.verified {
            ...
            for location in &candidate.members {
                match warehouse.head(location).await {
                    ...
                    Ok(_)
                    | Err(relify_storage::Error::ObjectStore(object_store::Error::NotFound {
                        ..
                    })) => {}
