        ...
        loop {
                ...
                .claim_shared_ivf(&descriptor, owner, SHARED_IVF_LEASE_MS)?
            {
                SharedIvfClaimResult::Ready(_) => {
                    return self
                        .load_shared_ivf(&fingerprint, &descriptor, prepared)
                        .await;
                }
        ...
        match result {
            ...
            Err(error) => {
                ...
                if let Ok(shared) = self
                    .load_shared_ivf(&claim.fingerprint, &descriptor, prepared)
                    .await
                {
                    return Ok(shared);
                }
