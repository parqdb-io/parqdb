...
use relify_meta::{
    ...
    RelationReference, SharedIvfDescriptor, SharedIvfMetadata, SnapshotLogEntry,
};
...
async fn repository_validates_the_shared_artifact_against_the_logical_snapshot() {
    ...
    let source_reference = source("file:///data/documents.parquet");
    let centroids = source("file:///indexes/shared/centroids");
    ...
    snapshot.source = source_reference;
    snapshot
        .index_relations
        .insert("ivf_centroids".into(), centroids);
    snapshot
        .parameters
