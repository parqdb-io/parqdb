...
fn derives_locations_for_each_supported_scheme() {
    let cases = [
        ("file:///tmp/relify", "file:///tmp/relify/indexes/a/"),
        ("s3://bucket/relify", "s3://bucket/relify/indexes/a/"),
        (
            "hdfs://namenode:8020/relify",
            "hdfs://namenode:8020/relify/indexes/a/",
        ),
    ];
...
fn rejects_locations_outside_the_warehouse() {
    let warehouse =
        Warehouse::open("file:///tmp/relify/", StorageRegistry::new(HashMap::new())).unwrap();
    assert!(warehouse.managed("file:///tmp/other/file").is_err());
    assert!(warehouse.managed("s3://bucket/relify/file").is_err());
}

#[test]
fn rejects_unsupported_or_noncanonical_locations() {
    let registry = StorageRegistry::default();
    assert!(Warehouse::open("gs://bucket/relify", registry.clone()).is_err());
    assert!(Warehouse::open("file://host/tmp/relify", registry).is_err());
}
...
fn derives_only_object_parents_below_the_warehouse_root() {
    let root = ObjectPath::from("relify");
    let object = ObjectPath::from(
        "relify/indexes/0123456789abcdef0123456789abcdef/1/ivf_postings/part.parquet",
    );
    assert_eq!(
        object_parents_below(&root, &object),
        [
            "relify/indexes/0123456789abcdef0123456789abcdef/1/ivf_postings",
            "relify/indexes/0123456789abcdef0123456789abcdef/1",
            "relify/indexes/0123456789abcdef0123456789abcdef",
            "relify/indexes",
        ]
        .map(ObjectPath::from)
    );
    assert!(object_parents_below(&root, &ObjectPath::from("outside/file")).is_empty());
    assert!(object_parents_below(&root, &ObjectPath::from("relify/file")).is_empty());
}
...
fn constructs_s3_and_hdfs_adapters_without_accessing_remote_storage() {
    ...
    .unwrap();
    assert_eq!(s3.root(), "s3://bucket/relify/");

    let hdfs = Warehouse::open("hdfs://namenode:8020/relify", StorageRegistry::default()).unwrap();
    assert_eq!(hdfs.root(), "hdfs://namenode:8020/relify/");
}
