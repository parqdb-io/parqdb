...
use parqdb_kernels::{LvqBits, detect};

use super::exec::DistanceInput;
use super::selector::compute_batch_distances;
use super::{IvfTopKExec, parqdb_session_context};
use crate::config::parqdb_session_config;
...
#[test]
fn parqdb_context_registers_internal_udfs_once_at_initialization() {
    let context = parqdb_session_context(
...
async fn fuses_distance_projection_topk_and_updates_its_dynamic_filter() {
    ...
    let dataframe = context
        .sql(
            "SELECT id, \
             parqdb_squared_l2(embedding, make_array(CAST(0 AS REAL), CAST(0 AS REAL))) \
             AS _distance \
