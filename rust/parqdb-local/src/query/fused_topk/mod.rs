...
use parqdb_kernels::LvqBits;
use parqdb_meta::DistanceMetric;

mod exec;
mod selector;

const DISTANCE_COLUMN: &str = "_distance";
const DISTANCE_UDF: &str = "parqdb_squared_l2";
const LVQ4_DISTANCE_UDF: &str = "parqdb_lvq4_l2";
const LVQ8_DISTANCE_UDF: &str = "parqdb_lvq8_l2";

#[cfg(test)]
mod tests;

/// Builds the single `DataFusion` context used by local `ParqDB` sessions.
pub(crate) fn parqdb_session_context(
    config: SessionConfig,
    runtime: Arc<RuntimeEnv>,
) -> SessionContext {
    let base = SessionContext::new_with_config_rt(config, runtime);
...
/// Replaces `Sort(Projection(parqdb_squared_l2(...)))` with one vector-native
