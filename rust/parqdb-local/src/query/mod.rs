...
use parqdb_meta::{DistanceMetric, IndexSnapshot, PostingEncoding};
...
pub(crate) use fused_topk::parqdb_session_context;
    ...
    fn invoke_with_args(
        ...
        let [vectors, queries] = arguments.args.as_slice() else {
            return Err(DataFusionError::Execution(
                "parqdb_squared_l2 requires exactly two arguments".into(),
            ));
        };
...
/// Returns the stateless squared-Euclidean distance UDF used by `ParqDB` queries.
#[must_use]
...
impl ScalarUDFImpl for LvqSquaredL2 {
    fn name(&self) -> &'static str {
        match self.bits {
            LvqBits::Four => "parqdb_lvq4_l2",
            LvqBits::Eight => "parqdb_lvq8_l2",
        }
    }
...
pub fn compile_datafusion_sql(
        ...
        && selected_clusters_name.is_none()
    {
        ...
        selected_clusters_name = Some("parqdb_selected_clusters");
    }
...
fn datafusion_distance_sql(resolved: &ResolvedSearch, query_literal: &str) -> String {
        ...
        || resolved.posting_encoding == PostingEncoding::Source
    {
        ...
        let vector = match resolved.metric {
            DistanceMetric::Cosine => format!("parqdb_normalize_vector({vector})"),
            DistanceMetric::L2Squared if resolved.source_vector_is_f64 => {
                format!("parqdb_vector_f32({vector})")
            }
            DistanceMetric::L2Squared => vector,
        };
        return format!("parqdb_squared_l2({vector}, {query})");
    }
    match resolved.posting_encoding {
        PostingEncoding::Lvq4 => {
            format!("parqdb_lvq4_l2(p.\"code\", p.\"offset\", p.\"scale\", {query})")
        }
        PostingEncoding::Lvq8 => {
            format!("parqdb_lvq8_l2(p.\"code\", p.\"offset\", p.\"scale\", {query})")
        }
...
fn datafusion_candidates_cte(
    ...
    if source_required {
        ...
        return Ok(format!(
            "parqdb_candidates AS (\n        SELECT s.*, {distance} AS _distance\n        \
             FROM parqdb_source AS s\n        JOIN parqdb_postings AS p ON {join}\n    )"
        ));
    }
...
fn datafusion_native_selected_clusters_cte(resolved: &ResolvedSearch) -> Result<String> {
    let rows = native_selected_clusters(resolved)?
        ...
        .join(", ");
    Ok(format!(
        "parqdb_selected_clusters(\"cid\") AS (\n        VALUES {rows}\n    )"
    ))
}
