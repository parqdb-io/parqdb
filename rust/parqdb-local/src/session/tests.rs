...
fn metadata_cache_config_is_shared_with_repository_handles() {
    ...
    let datafusion_config = parqdb_session_config()
...
async fn metadata_cache_config_tracks_datafusion_session_set() {
    ...
    session
        .context()
        .sql("SET parqdb.metadata.cache.max_entries = 7")
        .await
        .unwrap()
        .collect()
        .await
    ...
    session
        .context()
        .sql("SET parqdb.metadata.cache.max_bytes = 4096")
        .await
        .unwrap()
        .collect()
        .await
...
async fn parquet_page_cache_is_shared_across_sessions() {
    ...
    let runtime =
        Arc::new(ParqDBRuntime::new(RuntimeEnvBuilder::default(), Some(1024 * 1024)).unwrap());
    let first_session = LocalSession::open_with_options(
        temporary.path().join("first"),
        LocalSessionOptions::with_runtime(parqdb_session_config(), Arc::clone(&runtime)),
    )
    .unwrap();
    ...
    second_session
        .context()
        .sql("SET parqdb.parquet.page_cache.capacity = 0")
        .await
        .unwrap()
        .collect()
        .await
...
fn session_preserves_datafusion_config_and_runtime() {
    let temporary = TempDir::new().unwrap();
    let config = parqdb_session_config().with_target_partitions(3);
...
fn default_session_uses_bounded_fair_spill_pool() {
    ...
    let config = parqdb_session_config().set_str("parqdb.parquet.page_cache.capacity", "0");
    let session =
...
async fn managed_query_stream_holds_and_releases_admission() {
    let temporary = TempDir::new().unwrap();
    let runtime = Arc::new(
        ParqDBRuntime::with_query_admission(
