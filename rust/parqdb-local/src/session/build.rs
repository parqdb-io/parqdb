...
struct ClaimHeartbeat {
    stop: Sender<()>,
    task: JoinHandle<parqdb_catalog::Result<()>>,
}
        ...
        match result {
            Ok(centroid_artifact) => {
                ...
                if let Err(error) = heartbeat_result
                    && !matches!(
                        &error,
                        Error::Catalog(parqdb_catalog::Error::IvfCentroidsClaimLost(fingerprint))
...
impl ClaimHeartbeat {
    fn start(catalog: Arc<dyn IndexCatalog>, claim: IvfCentroidsClaim) -> Result<Self> {
        ...
        let task = thread::Builder::new()
            .name("parqdb-ivf-centroids-heartbeat".into())
            .spawn(move || {
                loop {
...
fn resolve_refresh_config(
    current: &parqdb_meta::IndexSnapshot,
