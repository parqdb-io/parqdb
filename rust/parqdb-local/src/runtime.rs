...
use crate::Result;
use crate::parquet::{
    DecompressedParquetPageCache, ParqDBParquetPageCacheFactory, ParquetPageCacheStats,
...
impl ParqDBRuntime {
    ...
    ///
    /// `ParqDB` reserves 80% of the effective memory limit for `DataFusion` execution
    ...
    pub fn with_query_admission(
        ...
        let parquet_page_cache_factory: Arc<dyn ParquetPageCacheFactory> = Arc::new(
            ParqDBParquetPageCacheFactory::new(Arc::clone(&parquet_page_cache), capacity),
        );
        Ok(Self {
            datafusion,
            parquet_page_cache,
