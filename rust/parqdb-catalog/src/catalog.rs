...
/// They are not part of portable `ParqDB` index metadata.
...
/// Runtime catalog operations used by `ParqDB`.
///
...
pub trait IndexCatalog: Send + Sync {
    ...
    ///
    /// Source discovery is a `ParqDB` runtime extension to the portable catalog
