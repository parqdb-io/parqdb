...
//!
//! `ParqDB` uses only the coarse-centroid path. Product-quantizer and
...
use parqdb_kernels::{KernelError, detect, row_dot_products};
use thiserror::Error;
...
impl KMeansOptions {
    /// Returns options with deterministic `ParqDB` defaults.
    #[must_use]
