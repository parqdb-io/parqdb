...
releasing the Relify distribution.
...
Relify defines a versioned Python extension API under `relify.backends.v1`.
One backend distribution registers one `BackendPlugin` through the
`relify.backends` package entry-point group. Registration is lazy: importing
Relify and listing installed entry points do not import third-party plugin
modules.
...
`relify.open_index_catalog` and `IndexCatalog.select` expose the catalog and
...
Reusable contract checks live under `relify.testing`, outside the
...
modifying `relify.__init__`. Ordinary users do not need to understand the
...
Relify now owns a compatibility surface for `relify.backends.v1`. A future
