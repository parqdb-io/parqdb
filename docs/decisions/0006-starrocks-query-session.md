...
ParqDB needs an OLAP reader that demonstrates one open Iceberg index can
...
query scheduling. ParqDB should supply portable index discovery and query
...
supports exact snapshot queries through `VERSION AS OF`. A ParqDB reader must
...
`parqdb.experimental.starrocks` is a third concrete session. It binds:
...
ParqDB's catalog lifecycle and may delegate construction to an independent
...
The ADBC connection remains owned by the caller. ParqDB opens and closes a
...
A conforming builder can publish canonical Iceberg index tables and ParqDB
metadata, while StarRocks reads the same exact table snapshots and executes the
complete IVF query in its own distributed runtime. Neither `parqdb-local` nor
...
`parqdb.experimental.starrocks`. A real integration test loads the shared specification
...
remote ParqDB index catalog, a maintained StarRocks/Iceberg conformance
