...
zero-copy `cid`-to-range directory. Queries select clusters in process and
create a query-local provider containing only range descriptors. Its scan
slices the shared Arrow batches lazily and accepts DataFusion runtime filters
only for `cid` and `key_i` columns. Uncached queries choose native or relational
cluster selection using the centroid-matrix heuristic, then expose a static or
dynamic cluster filter to the Parquet scan.
