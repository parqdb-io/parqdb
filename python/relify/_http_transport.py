...
class HttpTransport:
    """Versioned HTTP transport for the portable Relify service contract."""
        ...
        while True:
            ...
            if not isinstance(values, list):
                raise StreamExecutionError(
                    "Relify server returned invalid table metadata"
                )
            ...
            if not isinstance(next_token, str) or not next_token:
                raise StreamExecutionError(
                    "Relify server returned an invalid page token"
                )
            if next_token in seen_tokens:
                raise StreamExecutionError("Relify server repeated a table page token")
    ...
    async def explain(
        ...
        *,
        ...
        if not isinstance(plan, str):
            raise StreamExecutionError("Relify server returned an invalid query plan")
        return plan
    ...
    async def to_sql(self, query: VectorQuery) -> str:
        ...
        if not isinstance(sql, str):
            raise StreamExecutionError("Relify server returned invalid SQL")
        return sql
    ...
    async def _request_json(
        ...
        *,
        ...
        if not isinstance(body, dict):
            raise StreamExecutionError("Relify server returned invalid JSON")
        return body
    ...
    async def _request_stream(
        ...
        *,
        ...
        if content_type != ARROW_STREAM_MEDIA_TYPE:
            await response.aclose()
            raise StreamExecutionError(
                "Relify server returned an unexpected response type",
        ...
        try:
            ...
            await source.aclose()
            raise StreamExecutionError(
                "Relify query stream ended before its Arrow schema",
                request_id=request_id,
            ) from error
...
class _RemoteBatchStream:
    ...
    async def __anext__(self) -> pyarrow.RecordBatch:
        try:
            ...
            raise StreamExecutionError(
                "Relify query failed while streaming its result",
...
def _response_json(response: httpx.Response) -> object:
    if len(response.content) > _ERROR_BODY_LIMIT:
        raise StreamExecutionError("Relify server returned an oversized JSON response")
