...
def raise_remote_error(response: httpx.Response, body: object) -> None:
    code = "remote_error"
    message = f"ParqDB server returned HTTP {response.status_code}"
...
def raise_remote_build_error(code: str | None, message: str) -> None:
    error_type = _CLIENT_ERRORS.get(code or "", _native.ParqDBError)
    raise error_type(message)


def unavailable(error: httpx.HTTPError) -> ServiceUnavailableError:
    return ServiceUnavailableError(f"ParqDB server is unavailable: {error}")
