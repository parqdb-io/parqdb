...
if TYPE_CHECKING:
    ...
    from parqdb.datafusion.common import (  # type: ignore[import]
        DataTypeMap,
        NullTreatment,
        RexType,
    )
    from parqdb.datafusion.context import SessionContext
    from parqdb.datafusion.plan import LogicalPlan
...
class Expr:  # noqa: PLW1641
    ...
    def from_bytes(cls, buf: bytes, ctx: SessionContext | None = None) -> Expr:
        ...
        Examples:
            >>> from parqdb.datafusion import Expr, col, lit
            >>> blob = (col("a") + lit(1)).to_bytes()
            >>> Expr.from_bytes(blob).canonical_name()
            'a + Int64(1)'
        """
        from parqdb.datafusion.ipc import _resolve_ctx
...
class GroupingSet:
    ...
    def rollup(*exprs: Expr | str) -> Expr:
        ...
        Examples:
            >>> from parqdb.datafusion.expr import GroupingSet
            >>> ctx = dfn.SessionContext()
