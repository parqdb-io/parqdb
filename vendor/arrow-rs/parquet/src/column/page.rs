...
/// One complete, decompressed Parquet page and its physical file span.
///
/// The compressed range begins at the Page header and ends after the
/// compressed Page body. A cache uses the range end to advance a reader on a
/// hit without reading the Page header or body again.
#[derive(Clone, Debug)]
pub struct CachedPage {
    page: Page,
    compressed_range: Range<u64>,
    statistics_decoded: bool,
}

impl CachedPage {
    /// Creates a cacheable Page after validating its physical span.
    pub fn try_new(
        page: Page,
        compressed_range: Range<u64>,
        statistics_decoded: bool,
        ...
        if compressed_range.is_empty() {
            return Err(ParquetError::General(
                "cached Page compressed range must not be empty".into(),
            ));
        }
        Ok(Self {
            page,
    ...
    /// Returns the decompressed Page.
    pub fn page(&self) -> &Page {
        &self.page
    }

    /// Returns the Page's physical header-and-body range.
    pub fn compressed_range(&self) -> &Range<u64> {
        &self.compressed_range
    }

    /// Returns whether Page statistics were decoded with the Page header.
    pub fn statistics_decoded(&self) -> bool {
        self.statistics_decoded
    }

    /// Replaces the Page's backing buffer without changing its metadata.
    ///
    /// Cache implementations can use this to attach allocation-lifetime
    /// accounting to the returned [`Bytes`] owner without copying its payload.
    pub fn map_buffer(mut self, map: impl FnOnce(Bytes) -> Bytes) -> Self {
        let buffer = match &mut self.page {
            Page::DataPage { buf, .. }
            | Page::DataPageV2 { buf, .. }
            | Page::DictionaryPage { buf, .. } => buf,
        };
        *buffer = map(std::mem::take(buffer));
        self
    }

    pub(crate) fn metadata(&self) -> PageMetadata {
        match &self.page {
...
/// File-scoped lookup and admission hook for decompressed Parquet Pages.
///
/// Implementations bind this handle to a stable physical file identity before
/// constructing a Page reader. The `page_offset` passed to [`Self::get`] is the
/// physical offset of the Page header within that file.
pub trait PageCache: std::fmt::Debug + Send + Sync {
    /// Returns a resident Page whose compressed range starts at `page_offset`.
    fn get(&self, page_offset: u64) -> Option<Arc<CachedPage>>;

    /// Admits a fully validated Page and returns the canonical Page allocation
    /// that the current reader must consume.
    ///
    /// Implementations may return a query-owned entry without retaining it
    /// when admission is disabled or would exceed the cache budget.
