...
use crate::arrow::in_memory_row_group::{
    ColumnChunkData, ColumnChunkSource, FetchRanges, InMemoryRowGroup,
};
use crate::column::page::{CachedPage, PageCache};
...
pub(super) struct DataRequest {
    ...
    /// Complete cached Pages that cover each corresponding requested range.
    cached_pages: Vec<Option<Vec<Arc<CachedPage>>>>,
}

impl DataRequest {
    /// return what ranges are still needed to satisfy this request. Returns an empty vec
    /// if all ranges are satisfied
    pub fn needed_ranges(
        &mut self,
        buffers: &PushBuffers,
        ...
        for (index, range) in self.ranges.iter().enumerate() {
            if buffers.has_range(range) || self.cached_pages[index].is_some() {
                continue;
            }
            if let Some(page_cache) = page_cache
                && let Some(pages) = cached_pages_for_range(page_cache, range)?
            {
                self.cached_pages[index] = Some(pages);
                continue;
            }
            needed.push(range.clone());
        }
        Ok(needed)
    }

    /// Returns the chunks from the buffers that satisfy this request
    fn get_chunks(&self, buffers: &PushBuffers) -> Result<Vec<ColumnChunkSource>, ParquetError> {
        self.ranges
            .iter()
            .enumerate()
            .map(|(index, range)| {
                if let Some(pages) = &self.cached_pages[index] {
                    return Ok(ColumnChunkSource::Cached(pages.clone()));
                }
                ...
                buffers
                    .get_bytes(range.start, length)
                    .map(ColumnChunkSource::Bytes)
                    .map_err(|e| {
...
impl<'a> DataRequestBuilder<'a> {
    ...
    pub(crate) fn build(self) -> DataRequest {
        ...
        DataRequest {
            ...
            ranges,
            page_start_offsets,
            cached_pages: vec![None; range_count],
        }
    }
}

fn cached_pages_for_range(
    page_cache: &Arc<dyn PageCache>,
    range: &Range<u64>,
) -> Result<Option<Vec<Arc<CachedPage>>>, ParquetError> {
    let mut pages = Vec::new();
    let mut offset = range.start;
    while offset < range.end {
        let Some(page) = page_cache.get(offset) else {
            return Ok(None);
        };
        let page_range = page.compressed_range();
        if page_range.start != offset || page_range.end <= offset || page_range.end > range.end {
            return Err(ParquetError::General(format!(
                "cached Page range {page_range:?} does not cover requested range {range:?}"
            )));
        }
        offset = page_range.end;
        pages.push(page);
    }
    Ok((offset == range.end).then_some(pages))
}
