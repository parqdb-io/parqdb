...
use crate::column::page::{PageCache, PageIterator, PageReader};
...
impl<T> ArrowReaderBuilder<T> {
    ...
    /// Provides a file-scoped decompressed Page cache for the reader.
    pub fn with_page_cache(self, page_cache: Arc<dyn PageCache>) -> Self {
        Self {
...
impl<T: ChunkReader + 'static> ReaderPageIterator<T> {
    ...
    fn next_page_reader(&mut self, rg_idx: usize) -> Result<SerializedPageReader<T>> {
        ...
        Ok(match &self.page_cache {
            Some(page_cache) => page_reader.with_page_cache(Arc::clone(page_cache)),
            None => page_reader,
        })
    }
}
