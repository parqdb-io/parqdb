...
use crate::column::page::{CachedPage, Page, PageCache, PageMetadata, PageReader};
...
enum SerializedPageReaderState {
    Values {
        ...
        next_page_header: Option<BufferedPageHeader>,
...
pub struct SerializedPageReader<R: ChunkReader> {
    ...
    next_cached_page: Option<Arc<CachedPage>>,
}
...
impl<R: ChunkReader> SerializedPageReader<R> {
    ...
    pub fn new_with_properties(
        ...
        Ok(Self {
            ...
            context,
            page_cache: None,
            next_cached_page: None,
        })
    }

    /// Adds a file-scoped decompressed Page cache to this reader.
    pub fn with_page_cache(mut self, page_cache: Arc<dyn PageCache>) -> Self {
        // A cached Page must satisfy every reader configuration that can use
        // the same physical file and offset. Decode statistics on admission so
        // a later reader cannot observe a weaker cached representation.
        self.context.read_stats = true;
        ...
        match &mut self.state {
            SerializedPageReaderState::Values {
                ...
                loop {
                    ...
                    return if let Some(buffered) = next_page_header.as_ref() {
                        if let Ok(_page_meta) = PageMetadata::try_from(&*buffered.header) {
                            Ok(Some(buffered.page_offset))
                        } else {
                            // For unknown page type (e.g., INDEX_PAGE), skip and read next.
                            *next_page_header = None;
                            continue;
                        ...
                        *next_page_header = Some(BufferedPageHeader {
                            page_offset,
    ...
    fn page_cache(&self) -> Option<&Arc<dyn PageCache>> {
        self.context
            .supports_page_cache()
            .then_some(self.page_cache.as_ref())
            .flatten()
    }

    fn cached_page_for_next(&mut self) -> Result<Option<Arc<CachedPage>>> {
        if let Some(page) = &self.next_cached_page {
            return Ok(Some(Arc::clone(page)));
        }
        let Some(cache) = self.page_cache().cloned() else {
            return Ok(None);
        };
        let (page_offset, current_offset, remaining_bytes, expected_range, is_dictionary) =
            match &self.state {
                SerializedPageReaderState::Values {
                    offset,
                    remaining_bytes,
                    ...
                    (
                        next_page_header
                            .as_ref()
                            .map_or(*offset, |header| header.page_offset),
                        *offset,
                        *remaining_bytes,
                        None,
                        *require_dictionary,
                ...
                SerializedPageReaderState::Pages {
                    ...
                    let Some((location, is_dictionary)) = dictionary_page
                        .as_ref()
                        .map(|location| (location, true))
                        .or_else(|| page_locations.front().map(|location| (location, false)))
                    else {
                        return Ok(None);
                    };
                    let page_offset = u64::try_from(location.offset)?;
                    let page_length = u64::try_from(location.compressed_page_size)?;
                    (
                        page_offset,
                        page_offset,
                        page_length,
                        Some(page_offset..page_offset + page_length),
                        is_dictionary,
                    )
                }
            };
        let Some(page) = cache.get(page_offset) else {
            return Ok(None);
        };
        if self.context.read_stats && !page.statistics_decoded() {
            return Ok(None);
        }
        let range = page.compressed_range();
        if range.start != page_offset
            || range.end < current_offset
            || range.end - current_offset > remaining_bytes
            || expected_range
                .as_ref()
                .is_some_and(|expected| range != expected)
        {
            return Err(ParquetError::General(format!(
                "cached Page has an invalid physical range: expected offset {page_offset}, got {range:?}"
            )));
        }
        if page.page().is_dictionary_page() != is_dictionary {
            return Err(ParquetError::General(format!(
                "cached Page type does not match the Page at offset {page_offset}"
            )));
        }
        self.next_cached_page = Some(Arc::clone(&page));
        Ok(Some(page))
    }

    fn take_cached_page(&mut self) -> Result<Option<Arc<CachedPage>>> {
        let Some(page) = self.cached_page_for_next()? else {
            return Ok(None);
        };
        match &mut self.state {
            SerializedPageReaderState::Values {
                ...
                let bytes_to_advance = page.compressed_range().end - *offset;
                *offset = page.compressed_range().end;
                *remaining_bytes -= bytes_to_advance;
                *next_page_header = None;
                if page.page().is_data_page() {
                    *page_index += 1;
                } else {
        ...
        self.next_cached_page = None;
        Ok(Some(page))
    }

    fn admit_page(&self, page: Page, compressed_range: Range<u64>) -> Result<Page> {
        let Some(cache) = self.page_cache() else {
            return Ok(page);
        };
        let cached = CachedPage::try_new(page, compressed_range, self.context.read_stats)?;
        Ok(cache.insert(cached).page().clone())
    }
}

impl SerializedPageReaderContext {
    fn supports_page_cache(&self) -> bool {
        #[cfg(feature = "encryption")]
        {
            self.crypto_context.is_none()
        }
...
impl<R: ChunkReader> PageReader for SerializedPageReader<R> {
    fn get_next_page(&mut self) -> Result<Option<Page>> {
        loop {
            if let Some(page) = self.take_cached_page()? {
                return Ok(Some(page.page().clone()));
            }
            let (page, compressed_range) = match &mut self.state {
                SerializedPageReaderState::Values {
                    offset,
                    ...
                    let page_offset = next_page_header
                        .as_ref()
                        .map_or(*offset, |header| header.page_offset);
                    let mut read = self.reader.get_read(*offset)?;
                    let header = if let Some(buffered) = next_page_header.take() {
                        *buffered.header
                    } else {
                        let (header_len, header) = Self::read_page_header_len(
                            &self.context,
                ...
                SerializedPageReaderState::Pages {
                    ...
                    let page_offset = u64::try_from(front.offset)?;
                    (page, page_offset..page_offset + u64::try_from(page_len)?)
                }
            };

            return Ok(Some(self.admit_page(page, compressed_range)?));
        }
    }

    fn peek_next_page(&mut self) -> Result<Option<PageMetadata>> {
        if let Some(page) = self.cached_page_for_next()? {
            return Ok(Some(page.metadata()));
        }
        match &mut self.state {
            SerializedPageReaderState::Values {
                offset,
                ...
                loop {
                    ...
                    return if let Some(buffered) = next_page_header.as_ref() {
                        if let Ok(page_meta) = (&*buffered.header).try_into() {
                            Ok(Some(page_meta))
                        } else {
                            // For unknown page type (e.g., INDEX_PAGE), skip and read next.
                            *next_page_header = None;
    ...
    fn skip_next_page(&mut self) -> Result<()> {
        if self.take_cached_page()?.is_some() {
            return Ok(());
        }
        match &mut self.state {
            SerializedPageReaderState::Values {
                ...
                if let Some(buffered) = next_page_header.take() {
                    verify_page_size(
                        buffered.header.compressed_page_size,
                        buffered.header.uncompressed_page_size,
                        *remaining_bytes,
                    )?;
                    // The next page header has already been peeked, so just advance the offset
                    *offset += buffered.header.compressed_page_size as u64;
                    *remaining_bytes -= buffered.header.compressed_page_size as u64;
                } else {
                    let mut read = self.reader.get_read(*offset)?;
                    let (header_len, header) = Self::read_page_header_len(
                        &self.context,
