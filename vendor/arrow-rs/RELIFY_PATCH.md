...
Relify adds a generic, file-scoped `PageCache` hook to the standard Parquet
...
The vendored crate must not contain Relify index, query, capacity, eviction, or
file-identity policy. Those concerns remain in `relify-local`.
...
3. run the vendored crate tests and Relify's warm-read tests; and
