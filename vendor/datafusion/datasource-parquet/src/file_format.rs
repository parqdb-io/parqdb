...
impl FileFormat for ParquetFormat {
    ...
    async fn create_physical_plan(
        ...
        if let Some(page_cache) = state
            .config()
            .get_extension::<ParquetPageCacheFactoryConfig>()
        {
            page_cache.factory().update_config(state.config());
            source = source.with_parquet_page_cache_factory(Arc::clone(page_cache.factory()));
        }
