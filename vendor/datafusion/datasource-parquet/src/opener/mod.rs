...
use crate::{
    ...
    ParquetPageCacheFactory, apply_file_schema_type_coercions,
};
...
pub(super) struct ParquetMorselizer {
    ...
    /// Object-store namespace used to disambiguate identical object paths.
    pub(crate) object_store_url: Arc<str>,
    ...
    /// Optional factory for file-scoped decompressed Parquet Page caches.
    pub parquet_page_cache_factory: Option<Arc<dyn ParquetPageCacheFactory>>,
...
impl ParquetMorselizer {
    ...
    fn prepare_open_file(
        ...
        let page_cache = self
            ...
            .as_ref()
            .and_then(|factory| {
                factory.create_page_cache(&self.object_store_url, &partitioned_file)
            });
    ...
    fn build_stream(self) -> Result<BoxStream<'static, Result<RecordBatch>>> {
        ...
        let (decoder, pending_decoders, remaining_limit) = {
            ...
            let decoder_config = DecoderBuilderConfig {
                ...
                decoder_limit,
                page_cache: prepared.page_cache.as_ref(),
            };
...
mod test {
    ...
    impl ParquetMorselizerBuilder {
        ...
        fn build(self) -> ParquetMorselizer {
            ...
            ParquetMorselizer {
                partition_index: self.partition_index,
                object_store_url: Arc::from("test://"),
                projection,
                batch_size: self.batch_size,
                limit: self.limit,
                preserve_order: self.preserve_order,
                ...
                parquet_page_cache_factory: None,
                pushdown_filters: self.pushdown_filters,
