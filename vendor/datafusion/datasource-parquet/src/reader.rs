...
use parquet::column::page::PageCache;
...
/// Creates a file-scoped decompressed Parquet Page cache handle.
///
/// Implementations derive a stable physical-file identity from the supplied
/// [`PartitionedFile`]. The resulting handle is passed to the standard
/// arrow-rs Page reader; this interface does not change pruning or decoding.
pub trait ParquetPageCacheFactory: Debug + Send + Sync + 'static {
    /// Applies the configuration snapshot used to plan the current scan.
    fn update_config(&self, _config: &SessionConfig) {}

    /// Returns the Page cache handle for `file`, or `None` to bypass caching.
...
/// Session configuration extension for a [`ParquetPageCacheFactory`].
#[derive(Debug, Clone)]
pub struct ParquetPageCacheFactoryConfig {
    factory: Arc<dyn ParquetPageCacheFactory>,
}

impl ParquetPageCacheFactoryConfig {
    /// Creates a session extension backed by `factory`.
    pub fn new(factory: Arc<dyn ParquetPageCacheFactory>) -> Self {
        Self { factory }
    }

    /// Returns the configured factory.
    pub fn factory(&self) -> &Arc<dyn ParquetPageCacheFactory> {
        &self.factory
    }
}
