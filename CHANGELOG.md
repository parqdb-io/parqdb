...
- An HTTP transport and Python ASGI server using the same session facades,
  ...
  prefix allowlist.
- A `relify serve` deployment command and TOML configuration, with a safe
  loopback-only default template generated by `relify config init`.
...
- Server implementation is organized into `server`, `transport`, and `runtime`
  packages. The public `relify.server` factory remains unchanged.
